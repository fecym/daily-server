...
const addConsumeInfoCore = async (req, res) => {
  ...
  // 处理总消费
  let total_amount = Number(vehiclePrice) + Number(snacksPrice) + Number(transferAccounts) + Number(otherPrice) + Number(repastPrice) + Number(tripPrice) + Number(shoppingPrice) + Number(cosmeticPrice) + Number(lifePrice)
  // 值最后加上总额
